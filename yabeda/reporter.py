...
class Reporter:
    ...
    def report_pipeline(self, request: Request):
        logging.info('Reporting %s pipeline %s to #%s', request.project_path, request.pipeline_id, request.channel)
        ...
        if pipeline.dto.project.icon_url:
            ...
            pipeline = pipeline.replace_dto(
                project=pipeline.dto.project._replace(icon_url='https://{}/icon/{}'.format(request.host, hash))
            )
            logging.info("{} project icon is {}".format(pipeline.dto.project.key, pipeline.dto.project.icon_url))
        ts = self.slack.send_report(request.channel, self.formatter.format(request, pipeline.dto))
        ...
        if os.fork() == 0:
            try:
                ...
                while not pipeline.dto.is_finished():
                    time.sleep(delay)
                    pipeline, updated = self.gitlab.refresh_pipeline_jobs(pipeline)
                    if updated:
